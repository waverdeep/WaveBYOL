...
class Encoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, stride, filter_size, padding, feature_extractor_model, pretrain):
        ...
        self.adaptive_average_pooling = nn.AdaptiveAvgPool1d(1)
        self.adaptive_max_pooling = nn.AdaptiveMaxPool1d(1)

    def forward(self, x):
        out_cat = F.normalize(x, dim=-1, p=4)
        out = self.encoder(out_cat)
        ...
        # l2 normalization
        out_cat = F.normalize(out_cat, dim=-1, p=4)

        # feature extract layer
        out_feature = self.feature_extractor(out_cat)

        # torch.Size([2, 512, 11, 4])
        batch, channel, power, seq = out_feature.shape
        out_feature_reshape = out_feature.reshape((batch, channel * power, seq))

        out01 = self.adaptive_max_pooling(out_feature_reshape)
        out01 = out01.reshape(batch, channel * power)
        out02 = self.adaptive_average_pooling(out_feature_reshape)
        out02 = out02.reshape(batch, channel * power)
